...
This repository includes [`.github/workflows/blueprint-pattern-integrity.yml`](../../.github/workflows/blueprint-pattern-integrity.yml), [`.mlc-config.json`](../../.mlc-config.json), and [`scripts/check-doc-links.py`](../../scripts/check-doc-links.py) (per-file `markdown-link-check` with filesystem fallback for Linux false positives on non-`.md` paths).
