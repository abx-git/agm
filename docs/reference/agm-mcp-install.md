...
Public package **`@abx-hh/agm-cli`** — MCP server + CLI. Session prompts ship **LLMLingua-2 compressed** ([Microsoft LLMLingua-2](https://huggingface.co/spaces/microsoft/llmlingua-2)); full plaintext workflows live in the [agm](https://github.com/abx-git/agm) repo — npm publishes compressed golden path only.
